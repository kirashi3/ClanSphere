...
function cs_admin_menu()
{
    ...
    if (file_exists('mods/' . $recent_mod . '/manage.php') && $account['access_' . $recent_mod] >= 3)
    {
        ...
        $link_count++;
        $data['menu']['manage'] = cs_link($cs_lang['manage'] . ' ' . $sql_count, $recent_mod,'manage');
        $data['menu']['manage_active'] = $recent_action == 'manage' ? ' class="active"' : '';
        $data['if']['manage'] = true;
    ...
    if (file_exists('mods/' . $recent_mod . '/create.php') && $account['access_' . $recent_mod] >= 3 && $recent_mod != 'shoutbox')
    {
        ...
        $link_count++;
        $data['menu']['create'] = cs_link($cs_lang['create'], $recent_mod,'create');
        $data['menu']['create_active'] = $recent_action == 'create' ? ' class="active"' : '';
        $data['if']['create'] = true;
    ...
    if (file_exists('mods/' . $recent_mod . '/options.php') && $account['access_' . $recent_mod] >= 5)
    {
        ...
        $link_count++;
        $data['menu']['options'] = cs_link($cs_lang['options'], $recent_mod,'options');
        $data['menu']['options_active'] = $recent_action == 'options' ? ' class="active"' : '';
