...
if(isset($cs_post['submit']))
{
    ...
    if($cs_abcode['abcode_func'] == 'img' && empty($files['picture']['tmp_name']) && empty($cs_abcode['abcode_file']))
...
if(!empty($error) OR !isset($cs_post['submit']))
{
    ...
    $data['abcode']['clip']  =
        $cs_lang['max_width'] . $op_abcode['max_width'] . 'px; ' .
        $cs_lang['max_height'] . $op_abcode['max_height'] . 'px; ' .
        $cs_lang['max_size'] . cs_filesize($op_abcode['max_size']) . '; ' .
        $cs_lang['filetypes'] . $return_types;
